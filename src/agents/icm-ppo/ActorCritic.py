...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var, activation=nn.Tanh(), device='cpu'):
        ...
        # Adjust the linear layer to match the flattened input size
        self.flattened_input_layer = nn.Sequential(
            nn.Linear(192, n_latent_var),  # Adjust here based on the flattened size
            activation
        ).to(self.device)
    ...
    def evaluate(self, state, action):
        ...
        state_value = self.value_layer(body_output)
