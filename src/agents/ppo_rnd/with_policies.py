...
def run_episode(env, agent, state_dim, render, training_mode, t_updates, n_update):
    ...
    while not done:
        ...
        if done:           
            ...
            os.makedirs("logdir/ppo-rnd/for_policy/", exist_ok=True)
            print(f"episodes: {eps_time}")
            
            # Load existing scores from the JSON file, if it exists
            file_path = "logdir/ppo-rnd/for_policy/scores_ppornd.json"
            ...
            # If the seed functionality was tied to logging data per run, this can be removed or replaced
            existing_data['last_run'] = existing_data.get('last_run', []) + [total_reward]
